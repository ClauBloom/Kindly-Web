...
const zh: Record<string, string> = {
  ...
  'popup.danmakuMaxTotal': '弹幕处理上限（视频弹幕总量超过则跳过改写）',
  ...
  'onb.step4.desc': '强度、展示方式与更多偏好，之后随时可在设置页调整。',
  ...
  'opt.rewrite.retryCount': '失败重试次数（网络/解析失败，0 = 不重试）',
  'opt.rewrite.retryIntervalSec': '重试间隔秒数（网络失败退避基数）',
  'opt.rewrite.includeAuthor': '把评论者昵称一起发送（用于语境，可能涉及昵称隐私）',
  'opt.rewrite.hideOriginalComment': '改写前隐藏评论原文（加载即显示"重写中"，改写完成后替换）',
  'opt.rewrite.hideOriginalDanmaku': '改写前隐藏弹幕原文（弹幕显示"重写中"，改写完成后替换）',
  'opt.rewrite.thinking': '启用思考模式',
  'opt.rewrite.thinking.warn': '不推荐开启：思考模式会先推理再回答，改写响应显著变慢',
  'opt.rewrite.emojiToKaomoji': '自动将表情符号转为颜文字（如 😂 → (≧▽≦)，由 LLM 改写时处理）',
  'opt.rewrite.danmakuMaxTotal': '弹幕处理上限（视频弹幕总量超过则跳过弹幕改写）',
  'opt.rewrite.stylePreset': '输出风格（提示词预设，控制改写后的语气与用词）',
  'opt.rewrite.customStyles': '自定义提示词',
  'opt.rewrite.customStyle.name': '风格名称',
  'opt.rewrite.customStyle.prompt': '风格指令（将追加到改写提示词，如：句尾加"喵"）',
  'opt.rewrite.customStyle.add': '添加',
  'opt.rewrite.customStyle.delete': '删除',
  'opt.rewrite.customStyle.needName': '请填写风格名称',
  'opt.rewrite.customStyle.needPrompt': '请填写风格指令',
  'opt.rewrite.customStyle.nameTooLong': '风格名称不能超过 20 字',
  'opt.rewrite.customStyle.promptTooLong': '风格指令不能超过 500 字',
  'opt.rewrite.customStyle.deleted': '已删除该自定义提示词',
  'opt.rewrite.customStyle.empty': '暂无自定义提示词，可在下方添加',
  'opt.rewrite.dmSpeed': '弹幕处理速度（快速 = 高并发 + 小批，改写快但请求多）',
  'opt.rewrite.dmSpeedHint': '并发请求数 · 每批弹幕条数',
  'opt.rewrite.dmConcurrency': '并发请求数',
  'opt.rewrite.dmBatchSize': '每批弹幕条数',
